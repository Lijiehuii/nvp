<template>
	<view class="like-card" @click="goToVideoDetail(likeData.id)">
		<view class="img" v-if="likeData.cover">
			<van-image width="100%" height="100%" use-loading-slot :src="'https://images.weserv.nl/?url='
				+likeData.cover">
		...
		<view class="vider-info">
			...
			<view class="author">
				<image :src="'https://images.weserv.nl/?url='+likeData.author.userinfo.avatar"></image>
				{{likeData.author.userinfo.username}}
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		computed: {
			...
			duration() {
				this.$duration(this.likeData.duration)
			}
		},
		methods: {
			goToVideoDetail(id) {
